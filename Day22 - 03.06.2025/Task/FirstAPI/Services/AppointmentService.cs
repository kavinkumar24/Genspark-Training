...
namespace FirstAPI.Services
{
    public class AppointmentService : IAppointmentService
    {
     ...
     public async Task<Appointmnet> AddAppointment(AppointmentAddRequestDto appointment)
        ...
        public async Task<Appointmnet> UpdateAppointment(string appointmentId, AppointmentUpdateRequestDto appointmentDto)
        {
            try
            {
                ...
                var existingAppointment = await _appointmentRepository.Get(appointmentId);
                if (existingAppointment == null)
                    throw new InvalidOperationException($"Appointment with ID {appointmentId} not found.");
                
                existingAppointment.AppointmnetDateTime = appointmentDto.AppointmentDate;
                existingAppointment.Status = appointmentDto.Status;
                
                return await _appointmentRepository.Update(appointmentId, existingAppointment);
            }
            catch (Exception e)
            {
                throw new Exception($"Error updating appointment with ID {appointmentId}: {e.Message}");    
            }
        }
    }
}
