...
1. Atomicity - Treated as a single unit, which either completes entirely or does not happen at all
2. Consistency - one valid state to another valid state (after transaction)
3. Isolation - independely wihtout interface
4. Durability - changes are permenant

Why are Transactions Important? - to maintain data integrity
...
-----session 2-------------------------
BEGIN TRANSACTION ISOLATION LEVEL REPEATABLE READ;
UPDATE tbl_bank_accounts SET balance = 220 WHERE account_id = 1;
